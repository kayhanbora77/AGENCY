...
def parse_date(val) -> str | None:
    ...
    # MM/DD/YYYY or DD/MM/YYYY — try dateutil
    try:
        from dateutil import parser as dp

        return dp.parse(v, dayfirst=False).strftime("%Y-%m-%d")
    except Exception:
        pass
    return v
...
def fix_flight_number(val) -> str | None:
    ...
    'EK-0236' -> 'EK236'
    'LH 9448' -> 'LH9448'
    Strips leading zeros from numeric part.
    """
    if val is None:
        return None
    v = str(val).strip()
    if not v or v in ("NULL", "None", ""):
        return None
    m = re.match(r"^([A-Z0-9]{2,3})[\s\-]?(\d+)$", v.upper())
    if m:
        carrier = m.group(1)
        number = str(int(m.group(2)))  # strips leading zeros
...
def classify_columns(headers: list[str]):
    ...
    for h in headers:
        hl = h.strip().lower()
        # FlightDate1..N  OR  FlightDateLocal1..N  OR  DepartureDateLocal1..N
        if re.match(r"(flightdate|departuredatelocal|flightdatelocal)\d*$", hl):
            date_cols.add(h)
        # BILL_DATE, TK_ACMADM also are dates
        elif hl in ("bill_date", "tk_acmadm"):
            date_cols.add(h)
        # FlightNumber1..N
        elif re.match(r"flightnumber\d*$", hl):
            flight_cols.add(h)
    return date_cols, flight_cols
...
for sheet in sheet_names:
    ...
    date_cols, flight_cols = classify_columns(headers)
    print(f"  Date cols   : {sorted(date_cols)}")
    print(f"  Flight cols : {sorted(flight_cols)}")
    print(f"  Total cols  : {len(headers)}")
