...
def create_clean_view(con):
    # ── FIX: Materialize as temp table with row_id for stable batching ──────
    ...
    con.execute(f"""
        ...
        SELECT
            ...
            NULLIF(regexp_replace(replace(trim(upper(FlightNumber1)), ' ', ''), '{FLTNO_REGEX}', '\\1\\2'), '') AS FN1,
            NULLIF(regexp_replace(replace(trim(upper(FlightNumber2)), ' ', ''), '{FLTNO_REGEX}', '\\1\\2'), '') AS FN2,
            NULLIF(regexp_replace(replace(trim(upper(FlightNumber3)), ' ', ''), '{FLTNO_REGEX}', '\\1\\2'), '') AS FN3,
            NULLIF(regexp_replace(replace(trim(upper(FlightNumber4)), ' ', ''), '{FLTNO_REGEX}', '\\1\\2'), '') AS FN4,
            NULLIF(regexp_replace(replace(trim(upper(FlightNumber5)), ' ', ''), '{FLTNO_REGEX}', '\\1\\2'), '') AS FN5,
            NULLIF(regexp_replace(replace(trim(upper(FlightNumber6)), ' ', ''), '{FLTNO_REGEX}', '\\1\\2'), '') AS FN6,
...
def insert_target_table(con, paired_rows, rejection_rows):
    ...
    rej_bases = [p[1] for p in paired_rows]
    ...
    con.execute(f"""
        ...
        SELECT
            ...
            CAST(NULL AS VARCHAR)   AS PNRCRS,
            CAST(NULL AS VARCHAR)   AS PNRAirline,
            CAST(NULL AS VARCHAR)   AS Airlines,
            CAST(NULL AS VARCHAR)   AS AirlineCode,
            CAST(NULL AS VARCHAR)   AS TktNo,
            ...
            CAST(NULL AS VARCHAR)   AS FlightNumber6,
            ...
            CAST(NULL AS TIMESTAMP) AS FlightDate6,
            ...
            CAST(NULL AS VARCHAR)   AS Airport7,
            CAST(NULL AS INTEGER)   AS _rej_idx
        WHERE 1=0
    """)

    df_staging = df_out.copy()
    df_staging["_rej_idx"] = range(len(df_staging))
    con.execute("INSERT INTO _batch_staging SELECT * FROM df_staging")

    # ── NULL-safe conflict check — IS NOT DISTINCT FROM treats NULL==NULL as TRUE
    conflict_idxs = {
        int(r[0])
        for r in con.execute(f"""
            ...
            WHERE EXISTS (
                ...
                WHERE TRIM(s.PNRCRS)       = TRIM(t.PNRCRS)
                  AND TRIM(s.AirlineCode)  = TRIM(t.AirlineCode)
                  AND TRIM(s.TktNo)        = TRIM(t.TktNo)
                  AND TRIM(s.FlightNumber1) = TRIM(t.FlightNumber1)
                  ...
                  AND (s.FlightNumber2 IS NOT DISTINCT FROM t.FlightNumber2)
                  AND (s.FlightNumber3 IS NOT DISTINCT FROM t.FlightNumber3)
                  AND (s.FlightNumber4 IS NOT DISTINCT FROM t.FlightNumber4)
                  AND (s.FlightNumber5 IS NOT DISTINCT FROM t.FlightNumber5)
                  AND (s.FlightNumber6 IS NOT DISTINCT FROM t.FlightNumber6)
                  AND (CAST(s.FlightDate2 AS TIMESTAMP) IS NOT DISTINCT FROM CAST(t.FlightDate2 AS TIMESTAMP))
                  AND (CAST(s.FlightDate3 AS TIMESTAMP) IS NOT DISTINCT FROM CAST(t.FlightDate3 AS TIMESTAMP))
                  AND (CAST(s.FlightDate4 AS TIMESTAMP) IS NOT DISTINCT FROM CAST(t.FlightDate4 AS TIMESTAMP))
                  AND (CAST(s.FlightDate5 AS TIMESTAMP) IS NOT DISTINCT FROM CAST(t.FlightDate5 AS TIMESTAMP))
                  AND (CAST(s.FlightDate6 AS TIMESTAMP) IS NOT DISTINCT FROM CAST(t.FlightDate6 AS TIMESTAMP))
                  AND (s.Airport3 IS NOT DISTINCT FROM t.Airport3)
                  AND (s.Airport4 IS NOT DISTINCT FROM t.Airport4)
                  AND (s.Airport5 IS NOT DISTINCT FROM t.Airport5)
                  AND (s.Airport6 IS NOT DISTINCT FROM t.Airport6)
                  AND (s.Airport7 IS NOT DISTINCT FROM t.Airport7)
            )
        """).fetchall()
    }
...
def flush_rejections(con, rejection_rows: list):
    if not rejection_rows:
        return
    expected = 28  # 25 source cols + RejectionReason + RejectionDetail + RejectedAt
    padded = []
    for r in rejection_rows:
        if len(r) < expected:
            padded.append(r + [None] * (expected - len(r)))
        else:
            padded.append(r[:expected])
...
def process_batch(con, row_id_start, rejection_rows: list):
    # ── FIX: row_id pagination — stable, no overlap between batches ──────────
    df = con.execute(f"""
