...
TABLE_NAME = "YATRA"
CSV_DIR = Path("/home/kayhan/Desktop/Gelen_Datalar/YATRA/PROCEED/")
...
def load_csv_files(con) -> None:
    ...
    for i, csv_path in enumerate(csv_files):
        ...
        if i == 0:
            ...
            SELECT
                BookingRef,
                try_strptime(BookingDate, '%d %b %Y')::TIMESTAMP AS BookingDate,
                SuperPNR,
                PaxName,
                PNRNo,
                AirlineName,
                TicketNo,
                FlightNumber,
                try_strptime(DepartureDate, '%d-%m-%Y %I:%M %p')::TIMESTAMP AS DepartureDate,
                DepartureAirport,
                try_strptime(ArrivalDate, '%d-%m-%Y %I:%M %p')::TIMESTAMP AS ArrivalDate,
                ArrivalAirport
            FROM read_csv_auto(
                '{csv_path}',
                all_varchar=true,
            ...
            SELECT
                BookingRef,
                try_strptime(BookingDate, '%d %b %Y')::TIMESTAMP,
                SuperPNR,
                PaxName,
                PNRNo,
                AirlineName,
                TicketNo,
                FlightNumber,
                try_strptime(DepartureDate, '%d-%m-%Y %I:%M %p')::TIMESTAMP,
                DepartureAirport,
                try_strptime(ArrivalDate, '%d-%m-%Y %I:%M %p')::TIMESTAMP,
                ArrivalAirport
            FROM read_csv_auto(
                '{csv_path}',
                all_varchar=true,
